...
**Experiments confirm min_ops=5 is the practical optimum, not just a lower bound.** Table R5 (Part A) retrains from scratch with min_ops ∈ {3, 4, 5, 7}: values below the theoretical minimum (3 or 4) create coverage gaps that degrade performance (IID 80.73–81.51, OOD 52.29–52.32), while exceeding it (7) over-constrains the policy without benefit (IID 80.99, OOD 53.92). The default min_ops=5 achieves the best IID 85.15 / OOD 59.59 — a gap of at least 3.64 IID and 5.67 OOD over any alternative. Table R5 (Part B) further tests reward weight sensitivity: equal weighting (0.25×4) yields IID 82.29 (−2.86) / OOD 55.58 (−4.01); checker-heavy (0.4/0.2/0.2/0.2) yields IID 82.94 (−2.21) / OOD 54.41 (−5.18). All non-default configurations degrade gradually (IID −2.21 to −4.42, OOD −4.01 to −7.30) rather than catastrophically, confirming robustness while validating the default as optimal.

**The structural prior does not bias toward longer workflows — it teaches the policy *when to stop*.** Table R6 and Appendix H show FlowSteer generates task-proportional workflows: GSM8K uses a simple 3-operator chain in ~8.3 turns $`/`$ 21.9K tokens, while AIME 2025 constructs Plan→Programmer→Verify→Format over 12.4 turns $`/`$ 47.2K tokens. Spearman ρ = 0.65 (p = 0.021) confirms statistically significant adaptation to task difficulty. Crucially, Figure 5(a-b) shows CWRPO (Full) uses *fewer* tokens and turns than all ablation variants — removing the structural prior actually *increases* cost because the policy over-explores without curriculum guidance.
