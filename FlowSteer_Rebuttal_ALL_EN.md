...
We acknowledge that scaling to hundreds of APIs remains an open challenge for all current workflow orchestration methods. A promising direction is hierarchical operator selection (category → operator), which our factored action space naturally supports via category-level routing.
...
Vectorized rollout (Appendix G): 32-way concurrency, batched API calls, cached states, early-finish skip. ~300 steps, amortized ~1.3 GPU-hrs/backend. **Table R7** (new): 12→24 operators causes only −X.X IID / −X.X OOD degradation.
