...
Concretely, the factored action space (Section 4.1, Proposition 1) decouples action types from operators, yielding O(|A_type|+|O|) complexity — adding new operators grows cost linearly. Flow-Director selects operators through semantic descriptions (Table 7), not indices, so overlapping functionality is resolved by the policy's understanding of operator semantics rather than exhaustive search.
