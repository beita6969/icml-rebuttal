...
**Proposition 3（附录 B.2）**已提供形式化分析，包含四个子证明：(a) **符号分离**（公式 45–48）——不可行 $`R(\tau) \in [-1, 0)`$，可行 $`R(\tau) \geq 0`$；(b) **两阶段优化**（公式 49–56）——条件释放将概率质量转移至可行轨迹，$`p_\theta`$ 单调递增；(c) **有界更新**（公式 57–60）——裁剪 + KL 约束每步策略变化；(d) **Token 掩码方差缩减**（公式 61–66）——无偏估计且 $`\text{Var}(\text{mask}) < \text{Var}(\text{no-mask})`$。
...
FlowSteer 训练一次即可零样本部署到 6 个后端（Figure 4），无需逐后端调优。新**表 R6** 确认任务自适应推理成本：GSM8K 平均 8.3 轮 / \$0.0012，AIME 12.4 轮 / \$0.0019（Spearman ρ=0.65，p=0.021）。
