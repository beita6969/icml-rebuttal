...
感谢审稿人的积极评价。以下逐一回应各项关切，重点标注论文中已有的理论分析并补充新实验数据（完整数据见 [补充材料](https://anonymous.4open.science/r/Supplementary)）。

**W1/Q1：理论基础与奖励稳定性** *(新增：表 R1, R5 | 论文：Proposition 3, 附录 B.2)*

我们想强调，**Proposition 3（附录 B.2）**已提供条件释放机制的形式化分析，包含四个子证明：(a) **符号分离**（公式 45–48）——不满足结构约束的轨迹 $`R(\tau) \in [-1, 0)`$，满足的 $`R(\tau) \geq 0`$；(b) **两阶段优化**（公式 49–56）——条件释放可证明地将概率质量从不可行轨迹转移至可行轨迹，$`p_\theta`$ 单调递增（公式 55）；(c) **有界更新**（公式 57–60）——裁剪 + KL 联合约束每步策略变化；(d) **Token 掩码方差缩减**（公式 61–66）——无偏估计且 $`\text{Var}(\text{mask}) < \text{Var}(\text{no-mask})`$。
...
这些下降验证了理论预测：Token 掩码的方差缩减（Proposition 3d）对需要干净梯度信号的数学推理至关重要（MATH −13.28）；条件释放的两阶段优化（Proposition 3b）防止策略在长链 QA 任务上过早收敛（TriviaQA −9.38）；多样性奖励防止模式坍缩，对未见分布的影响最大（OOD −9.28）。三种失效模式相互正交，证实 CWRPO 是最小完备设计——每个机制对应一个理论上不同的挑战。

**奖励设计对超参数选择具有鲁棒性。** 表 R5 在保持所有机制激活的条件下测试 5 种配置：等权重（0.25×4）达 IID 82.29 / OOD 55.58；Checker‑heavy（0.4/0.2/0.2/0.2）达 IID 82.94 / OOD 54.41；min_ops ∈ {3, 4, 7} 达 IID 80.73–81.51 / OOD 52.29–53.92——所有变体均为渐进退化（IID −2.21 至 −4.42，OOD −4.01 至 −7.30）vs. 默认 IID 85.15 / OOD 59.59，证实对奖励塑形选择的鲁棒性。

**W2/Q2：消融粒度——解耦各贡献** *(新增：表 R1 | 论文：Tables 3, 5, 6)*
...
**W3/Q3：与 RL agent 框架的定位** *(论文：附录 E Table 12, Proposition 2)*
...
**W4/Q4：可扩展性与计算成本** *(新增：表 R6 | 论文：附录 C, G)*

**FlowSteer 的多轮开销是可控的。** 附录 C 显示训练复杂度 $`O(NT)`$、推理复杂度 $`O(T)`$，多轮成本通过向量化 rollout（**附录 G**）缓解：32 路并发轨迹交互、批量 API 调用、缓存工作流状态、提前结束跳过。
...
**Q5：跨后端泛化** *(新增：表 R2–R4 | 论文：Figure 4, Section 5.3)*
